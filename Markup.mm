...
+ (void) addMarkupMenuItem
{
    ...
    while (NSDictionary* markupStyleDefinition = [e nextObject])
    {
        ...
        NSObject<NSMenuItem>* markupMenuItem =
    ...
    NSObject<NSMenuItem>* useSmartQuotesMenuItem =
        [markupLanguagesMenu addItemWithTitle:@"Use Smart Quotes"
                                       action:@selector(onToggleSmartQuotes:)
                                keyEquivalent:@""];
    [useSmartQuotesMenuItem bind:@"value" toObject:[self sharedMarkupPlugin] withKeyPath:@"usingSmartQuotes" options:nil];
    
    [[self sharedMarkupPlugin] setValue:[NSNumber numberWithInt:NSOnState] forKey:@"usingSmartQuotes"];
...
+ (NSArray*) markupStyles
{
    ...
    {
        NSString* filterStylesPropertyListPath = [bundle pathForResource:@"MarkupFilters"
                                                                  ofType:@"plist"
...
+ (NSNumber*) markupEnabledForFilterStyleInSelectedRange:(NSString*)markupStyleName
{
    ...
    if ([firstResponder respondsToSelector:@selector(markupEnabledForFilterStyleInSelectedRange:)])
        return [firstResponder performSelector:@selector(markupEnabledForFilterStyleInSelectedRange:) withObject:markupStyleName];
    else
        return nil;
}
