 ...
 * @package    Texy
 * @category   Text
 * @version    1.1b for PHP4 & PHP5 $Date: 2007-02-13 06:38:32 +0100 (Tue, 13 Feb 2007) $ $Revision: 49 $
 */if(version_compare(PHP_VERSION,'4.3.3','<'))die('Texy!: too old version of PHP!');define('TEXY','Version 1.1b for PHP4 & PHP5 $Revision: 49 $');define('TEXY_DIR',dirname(__FILE__).'/');
...
TexyLinkElement($this->texy);$elLink->setLinkRaw($mLink,$mContent);return$lineParser->element->appendChild($elLink,$mContent);}function
...
setLinkRaw($link,$text=''){if(@$link{0}=='['&&@$link{1}!='*'){$elRef=&$this->texy->linkModule->getReference(substr($link,1,-1));if($elRef){$this->modifier->copyFrom($elRef->modifier);$link=$elRef->URL.$elRef->query;$link=str_replace('%s',urlencode(Texy::wash($text)),$link);}else{$this->setLink(substr($link,1,-1));return;}}$l=strlen($link);if(@$link{0}=='['&&@$link{1}=='*'){$elImage=&new
...
TexyLinkElement($this->texy);$el->setLinkRaw($mLink,$mContent);$mContent=$lineParser->element->appendChild($el,$mContent);}if($el)$el->modifier->setProperties($mMod1,$mMod2,$mMod3);return$mContent;}function
...
extends
TexyModule{var$doubleQuotes=array('&#8222;','&#8220;');var$singleQuotes=array('&#8218;','&#8216;');function
...
wash($text){return
preg_replace('#[\x15-\x1F]+#','',$text);}function
